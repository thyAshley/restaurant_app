    ...
    try {
      ...
      if (restaurant) {
        ...
        if (
          file[0] &&
          restaurant &&
          restaurant.ambienceSeats &&
          restaurant.ambienceSeats[0]
        )
          fs.unlinkSync(
            path.join(
              __dirname,
        ...
        restaurant!.ambienceSeats = [
          file[0]?.filename ||
            (restaurant.ambienceSeats ? restaurant.ambienceSeats[0] : null),
          file[1]?.filename ||
            (restaurant.ambienceSeats ? restaurant.ambienceSeats[1] : null),
          file[2]?.filename ||
            (restaurant.ambienceSeats ? restaurant.ambienceSeats[2] : null),
        ];
        const result = await restaurant.save();
        return res.send(result);
      }
